...
CNI_NETWORK_CONFIG_FILENAME='/host/etc/cni/net.d/10-kubestack.conf'
# If specified, overwrite the network configuration file.
if [ "${CNI_NETWORK_CONFIG:-}" != "" ]; then
cat >$CNI_NETWORK_CONFIG_FILENAME <<EOF
${CNI_NETWORK_CONFIG:-}
EOF
echo "Wrote CNI config: $(cat /host/etc/cni/net.d/${CNI_NETWORK_CONFIG_FILENAME})"
fi

KUBESTACK_CONFIG_FILENAME='/etc/kubestack.conf'
# If specified, overwrite the kubestack configuration file.
if [ "${KUBESTACK_CONFIG:-}" != "" ]; then
cat >$KUBESTACK_CONFIG_FILENAME <<EOF
${KUBESTACK_CONFIG:-}
EOF
echo "Wrote CNI config: $(cat /host/etc/cni/net.d/${KUBESTACK_CONFIG_FILENAME})"
fi
